  ...
  return (
    <div className="page">
      <PageHeader
        ...
        description="This page is now connected to the live read-only Solpient Research contract. It only reads latest published research; drafts and review-workbench records are not exposed."
      ...
      <div className="research-architecture">
        <section className="card page-card architecture-card">
          <Database size={24} />
          <div><strong>Solpient Money</strong><span>Demo accounts · transactions · holdings</span></div>
        </section>
        <span className="architecture-arrow">→</span>
        <section className="card page-card architecture-card live-contract">
          <ShieldCheck size={24} />
          <div><strong>money_research_snapshots_v1</strong><span>Security-invoker · read-only · published research</span></div>
        </section>
        <span className="architecture-arrow">→</span>
        <section className="card page-card architecture-card">
          <strong>Portfolio Intelligence</strong>
          <span>Scores · valuation · thesis · evidence · risks · changes</span>
        </section>
      </div>

      {!research.connected ? (
        <section className="card page-card error-card">
          <strong>Live Research unavailable</strong>
          <p>{research.error ?? "The Research read contract could not be reached."}</p>
        </section>
      ) : null}

      <section className="card page-card">
        <div className="section-title-row">
          <div><span className="card-kicker">LIVE PUBLISHED COVERAGE</span><h2>Portfolio companies</h2></div>
          <span className="small-muted">{covered.length} live matches</span>
        </div>
      ...
      <section className="card page-card">
        ...
        <div className="contract-facts">
          ...
          <div><strong>Current evidence layer</strong><span>Evidence confidence and coverage readiness come from the current Research ranking/coverage engines and carry separate timestamps.</span></div>
          <div><strong>No draft leakage</strong><span>Money does not read baseline drafts, composer output, review workbench records, or unpublished research runs.</span></div>
